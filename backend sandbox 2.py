    ...
    for link in links:
        ...
        for row in complete_rows:
            ...
            if normalized_row_link == link:
                ...
                image_filenames = [os.path.join(f'images/{listing_id}', image) for image in images]
                if not images:
                ...
                listing = {
                    ...
                    "area": row[3],
                    "image_filenames": image_filenames,  # Use list of image filenames
                    "bathrooms": row[6],
                    "renovation": row[7],
                    "build_period": row[8],
...
def index():
    for listing in listings:
        listing['image_url'] = url_for('static', filename=listing['image_filenames'][0])  # Set the first image URL for the index page
...
def listing_detail(listing_id):
    ...
    if listing:
        listing['image_urls'] = [url_for('static', filename=filename) for filename in listing['image_filenames']]
